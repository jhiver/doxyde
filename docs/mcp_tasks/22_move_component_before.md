# Task 22: Move Component Before Tool

## Overview
Implement the `move_component_before` tool to move a component before another component in the same draft version.
...
5. **Not Self**: Cannot move component before itself
...
- Trying to move component before itself

## Behavior

1. Moves component to position immediately before target
...
- Moving "before" means the component will have a lower position number

## Testing

```rust
#[cfg(test)]
mod move_component_before_tests {
    use super::*;
    ...
    async fn test_move_component_before_self_fails(pool: SqlitePool) -> Result<()> {
        ...
        let req = MoveComponentBeforeRequest {
            component_id,
            before_component_id: component_id,
        };
    ...
    async fn test_move_component_different_versions_fails(pool: SqlitePool) -> Result<()> {
        // Create components in different page versions
        let comp1_id = 100; // in version 1
        let comp2_id = 101; // in version 2
        let service = DoxydeRmcpService::new(pool, 1);

...
static SLUG_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"[^a-zA-Z0-9]+").expect("Failed to compile slug regex")
});
...
pub fn generate_slug_from_title(title: &str) -> String {
    ...
    if slug.len() > 100 {
        slug = slug.chars().take(100).collect::<String>().trim_end_matches('-').to_string();
    }
...
mod tests {
    ...
    fn test_generate_slug_special_characters() {
        ...
        assert_eq!(generate_slug_from_title("Email@example.com"), "email-example-com");
    }

    #[test]
    fn test_generate_slug_whitespace() {
        assert_eq!(generate_slug_from_title("  Hello  World  "), "hello-world");
        assert_eq!(generate_slug_from_title("Multiple   Spaces"), "multiple-spaces");
        assert_eq!(generate_slug_from_title("\tTabs\tand\tSpaces\t"), "tabs-and-spaces");
    }
    ...
    fn test_generate_slug_consecutive_special_chars() {
        ...
        assert_eq!(generate_slug_from_title("Multiple!!!Exclamations"), "multiple-exclamations");
    }
}
...
mod security_tests {
    ...
    fn test_xss_prevention_in_templates() {
        ...
        for input in dangerous_inputs {
            assert!(input.contains('<') || input.contains("javascript:") || input.contains("';"),
                   "Input should contain dangerous pattern: {}", input);
        }
    }
    ...
    fn test_file_upload_security() {
        ...
        for file in dangerous_files {
            assert!(is_dangerous_filename(file), "Failed to detect dangerous file: {}", file);
            assert!(validate_upload_filename(file).is_err(), "Failed to reject dangerous file: {}", file);
        }
        ...
        for file in safe_files {
            assert!(!is_dangerous_filename(file), "Incorrectly flagged safe file: {}", file);
            assert!(validate_upload_filename(file).is_ok(), "Incorrectly rejected safe file: {}", file);
        }
    }
    ...
    fn test_null_byte_injection() {
        ...
        for attempt in null_byte_attempts {
            assert!(validate_upload_filename(attempt).is_err(), 
                   "Failed to reject null byte injection: {:?}", attempt);
        }
    }
    ...
    fn test_security_headers() {
        ...
        assert!(true, "Security headers are configured in security_headers.rs");
    }
}
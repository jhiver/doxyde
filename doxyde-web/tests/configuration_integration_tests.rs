...
"#,
    ...
    assert_eq!(
        merged.server.as_ref().unwrap().host,
        Some("0.0.0.0".to_string())
    ); // from etc
    assert_eq!(
        merged.session.as_ref().unwrap().session_timeout_minutes,
        Some(1080)
    ); // home override
    assert_eq!(
        merged.upload.as_ref().unwrap().max_upload_size,
        Some(15728640)
    ); // home override
    assert_eq!(
        merged
    ...
    assert_eq!(
        config.security.headers.csp_content,
...
"#
    ...
    assert_eq!(
        toml_config.server.as_ref().unwrap().host,
...
fn test_toml_serialization_roundtrip() -> Result<()> {
    ...
    assert_eq!(
        parsed_toml.server.as_ref().unwrap().host,
        Some("127.0.0.1".to_string())
    );
    ...
    assert_eq!(
        parsed_toml.session.as_ref().unwrap().secure_cookies,
        Some(false)
    );
    assert_eq!(
        parsed_toml.upload.as_ref().unwrap().max_upload_size,
        Some(20971520)
    );
    assert_eq!(
        parsed_toml
...
"#
    ...
    assert_eq!(
        headers.security_hsts_content.as_ref().unwrap(),
        "max-age=63072000; includeSubDomains; preload"
    );
    assert_eq!(
        headers.security_frame_options_content.as_ref().unwrap(),
        "SAMEORIGIN"
    );
    assert_eq!(
        headers.security_referrer_policy.as_ref().unwrap(),

...
mod tests {
    ...
    fn test_all_templates_compile() {
        ...
        for name in &template_names {
            ...
            match tera.render(name, &context) {
                Ok(_) => println!("✓ Template '{}' rendered successfully with minimal context", name),
                Err(e) => {
    ...
    fn test_mobile_templates_compile() {
        ...
        let mobile_templates = vec![
            ("mobile_header.html", include_str!("../../templates/mobile_header.html")),
            ("mobile_nav_drawer.html", include_str!("../../templates/mobile_nav_drawer.html")),
            ("mobile_edit_drawer.html", include_str!("../../templates/mobile_edit_drawer.html")),
        ];
        ...
        for (i, context) in test_contexts.iter().enumerate() {
            for (name, _) in &mobile_templates {
                match tera.render(name, context) {
                    Ok(_) => {},
                    Err(e) => println!("Warning: Template '{}' with context {} failed: {}", name, i, e),
                }
            }
        }
    }
